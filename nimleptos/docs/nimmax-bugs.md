...
| # | Bug | Severity | Status |
|---|-----|----------|--------|
| 1 | `[]=`/`[]`/`hasKey` infinite recursion on `TableRef[string, string]` | Critical | Workaround in NimLeptos |
| 2 | `escapeHtml` name collision with other modules | Medium | Workaround via qualified imports |
| 3 | WebSocket implementation is a stub (no real framing) | Medium | Not functional |
| 4 | `runOnce` missing `postParams`/`queryParams` | Low | Manual context construction |
| 5 | `parseCookies` uses overridden `[]=` | Low | May work by compilation order |
| 6 | `randomString` doesn't seed RNG | Low | Deterministic output |
| 7 | `escapeHtml` inconsistency with stdlib | Low | Cosmetic |

**Bug #1 is the most critical** — it makes `TableRef[string, string]` unusable in any module that imports nimmax, which is a fundamental issue for a web framework that deals extensively with string-keyed data.
